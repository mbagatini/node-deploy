...
import EnviaEmailAtivarCadastroService from './EnviaEmailAtivarCadastroService';
...
describe('Envia e-mail de ativação do cadastro', () => {
  ...
  let enviaEmailAtivarCadastro: EnviaEmailAtivarCadastroService;
  let cidade: Cidade;
  let estado: Estado;
  let usuario: Usuario;
  beforeEach(async () => {
    ...
    enviaEmailAtivarCadastro = new EnviaEmailAtivarCadastroService(
      fakeProvedorEmail,
  ...
  it('Deve poder enviar email de ativacao de cadastro para um usuário criado', async () => {
    const { email } = usuario;

    const enviouEmail = jest.spyOn(fakeProvedorEmail, 'enviaEmail');

    await enviaEmailAtivarCadastro.executa(email);
  ...
  it('Não deve tentar enviar email de ativacao de cadastro quando não encontrar o usuário do email', async () => {
    ...
    await expect(
      enviaEmailAtivarCadastro.executa(email),
